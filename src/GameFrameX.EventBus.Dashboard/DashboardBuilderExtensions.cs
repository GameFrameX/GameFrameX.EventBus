using GameFrameX.EventBus.Dashboard.Areas;
using Microsoft.Extensions.FileProviders;
using Microsoft.Extensions.Options;

namespace GameFrameX.EventBus.Dashboard;
...
public static class DashboardBuilderExtensions
{
    ...
    /// <returns></returns>
    public static IEventBusBuilder AddDashboard(this IEventBusBuilder builder, EventBusDashboardOption option = null)
    {
        if (option == null)
        {
    ...
    /// <returns></returns>
    public static IEventBusBuilder AddDashboard(this IEventBusBuilder builder, Action<EventBusDashboardOption> optionAction)
    {
    ...
    /// <returns></returns>
    public static IEventBusBuilder AddDashboard<TAuth>(this IEventBusBuilder builder, string prefix = null) where TAuth : class, IEventBusDashboardAuthentication
    {
    ...
    public static IApplicationBuilder UseEventBusDashboard(this IApplicationBuilder app)
    {
        ...
        app.UseStaticFiles(new StaticFileOptions
        {
            FileProvider = new EmbeddedFileProvider(typeof(DashboardBuilderExtensions).Assembly,
                                                    "GameFrameX.GameFrameX.EventBus.Dashboard.Resource"),
            RequestPath = option.UrlPrefix + "/static",
        });
        app.UseEndpoints(e =>
                         {
                             e.MapAreaControllerRoute(Consts.AreaName,
                                                      Consts.AreaName,
                                                      option.UrlPrefix + "/{controller=Published}/{action=Index}/{id?}");
                         });
        return app;
    }
}